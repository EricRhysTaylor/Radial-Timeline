% Generated from DesignedStyleSpec v2 — bundled-fiction-classic-manuscript
% Do not edit by hand. Source of truth: src/publishing/designedStyle.ts
% Archetype: submission

\documentclass[11pt,oneside]{book}

\usepackage{fontspec}
\usepackage{fancyhdr}
\usepackage{titlesec}
\usepackage{geometry}
\usepackage{setspace}
\usepackage{etoolbox}
\usepackage{emptypage}

\providecommand{\tightlist}{%
  \setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}
}

\geometry{
  paperwidth=6in,
  paperheight=9in,
  top=1in,
  bottom=1in,
  left=1in,
  right=1in
}

\defaultfontfeatures{Ligatures=TeX}
\IfFontExistsTF{Arial}{%
  \setmainfont{Arial}%
}{%
  \PackageError{rt-font}{Required font 'Arial' is not installed.\MessageBreak Install the font and re-run the export.}{%
    The Radial Timeline template requires this font and will not fall back to a substitute.\MessageBreak See Settings > Publish in Obsidian for install help.%
  }%
}

$if(title)$$else$\errmessage{Radial Timeline export requires Pandoc metadata: title}$endif$
$if(author)$$else$\errmessage{Radial Timeline export requires Pandoc metadata: author}$endif$
\newcommand{\BookTitle}{$if(title)$$title$$endif$}
\newcommand{\AuthorName}{$if(author)$$for(author)$$author$$sep$, $endfor$$endif$}
\newcommand{\rtSceneRunningTitle}{}
\newcommand{\rtSetSceneRunningTitle}[1]{\gdef\rtSceneRunningTitle{#1}\markboth{\BookTitle}{#1}}
\fancypagestyle{rtEmpty}{\fancyhf{}\renewcommand{\headrulewidth}{0pt}\renewcommand{\footrulewidth}{0pt}}
\fancyhf{}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\footrulewidth}{0pt}
\fancyhead[C]{\BookTitle}
\pagestyle{fancy}

\fancyfoot[C]{\thepage}

\pagenumbering{roman}
\newif\ifrtMainStarted
\rtMainStartedfalse
\newcommand{\rtBeginMainArabic}{%
  \clearpage
  \pagenumbering{arabic}%
  \setcounter{page}{1}%
  \rtMainStartedtrue%
}

\setcounter{secnumdepth}{0}
\makeatletter
\newcommand{\rtSceneOpenerTitle}[1]{%
  \begingroup
  \def\rt@sceneFirst{}%
  \rt@sceneFirstWord#1 \@nil
  \rt@sceneFirst
  \endgroup
}
\def\rt@sceneFirstWord#1 #2\@nil{\def\rt@sceneFirst{#1}}
\makeatother
\newcommand{\rtSceneSep}{%
  \clearpage
  \thispagestyle{empty}%
  \vspace*{2in}%
  \begin{center}{\Large * * *}\end{center}
  \vspace*{1in}%
}
\newcommand{\rtSceneOpener}[1]{%
  \ifrtMainStarted\else\rtBeginMainArabic\fi%
  \clearpage
  \null%
  \thispagestyle{empty}%
  \vspace*{0.16\textheight}%
  \begin{center}{\normalfont\bfseries\Large \rtSceneOpenerTitle{#1}}\end{center}
  \vspace*{0.12\textheight}%
}

\onehalfspacing
\usepackage{microtype}
\setlength{\emergencystretch}{3em}
\tolerance=1000
\hyphenpenalty=200
\setlength{\parindent}{1.5em}

\begin{document}

$body$

\end{document}
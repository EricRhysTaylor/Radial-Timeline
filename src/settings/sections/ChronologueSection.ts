import { App, Setting as Settings, Notice, DropdownComponent, ColorComponent, TextComponent } from 'obsidian';
import type RadialTimelinePlugin from '../../main';
import type { TimelineItem } from '../../types';
import { parseDurationDetail, formatDurationSelectionLabel, calculateAutoDiscontinuityThreshold, parseDateRangeInput } from '../../utils/date';
...
export function renderChronologueSection(params: { app: App; plugin: RadialTimelinePlugin; containerEl: HTMLElement; }): void {
    ...
    // 4. Backdrop micro-rings
    const microRingHeading = new Settings(containerEl)
        .setName('Backdrop micro-rings')
        .setHeading();
    addHeadingIcon(microRingHeading, 'layers-3');
    ...
    const ensureMicroRingConfigs = () => {
        const defaults = DEFAULT_SETTINGS.chronologueBackdropMicroRings || [
            { title: '', range: '', color: '#ffffff' },
            { title: '', range: '', color: '#ffffff' },
            { title: '', range: '', color: '#ffffff' }
        ];
        const current = Array.isArray(plugin.settings.chronologueBackdropMicroRings)
            ? plugin.settings.chronologueBackdropMicroRings
            : [];
        return Array.from({ length: 3 }, (_, i) => ({
            title: current[i]?.title ?? '',
            range: current[i]?.range ?? '',
            color: current[i]?.color ?? defaults[i]?.color ?? '#ffffff'
        }));
    };

    const updateMicroRing = async (index: number, patch: Partial<{ title: string; range: string; color: string }>) => {
        const next = ensureMicroRingConfigs();
    ...
    const microRingConfigs = ensureMicroRingConfigs();
    microRingConfigs.forEach((config, index) => {
        const ringLabel = `Micro ring ${index + 1}`;

        const rangeSetting = new Settings(containerEl)
            .setName(ringLabel)
            .setDesc('Title + date range (e.g., "4/24/2024-4/25/2025 1:45pm").');

        rangeSetting.addText(titleInput => {
            titleInput.setPlaceholder('Title')
                .setValue(config.title || '');
            titleInput.inputEl.classList.add('rt-input-md');
            titleInput.onChange(async (value) => {
                await updateMicroRing(index, { title: value });
            });
        });

        rangeSetting.addText(rangeInput => {
            rangeInput.setPlaceholder('Start - End')
                .setValue(config.range || '');
            rangeInput.inputEl.classList.add('rt-input-lg');

            const validateRange = () => {
                rangeInput.inputEl.removeClass('rt-setting-input-error');
                const trimmed = rangeInput.getValue().trim();
                if (!trimmed) return;
                const parsed = parseDateRangeInput(trimmed);
                if (!parsed?.start || !parsed?.end) {
                    rangeInput.inputEl.addClass('rt-setting-input-error');
                }
            };

            plugin.registerDomEvent(rangeInput.inputEl, 'blur', () => {
                validateRange();
            });

            rangeInput.onChange(async (value) => {
                await updateMicroRing(index, { range: value });
            });
        });

        const colorSetting = new Settings(containerEl)
            .setName(`${ringLabel} color`)
        ...
        colorPickerRef = new ColorComponent(colorSetting.controlEl)
            ...
            .onChange(async (value) => {
                ...
                await updateMicroRing(index, { color: normalized });

...
1. **Shell + Base Class**
   ...
   - Place major sections directly inside `.rt-gossamer-score-modal` (or `.rt-pulse-modal`) as bordered sections (e.g., `.rt-glass-card.rt-sub-card` style: light border, no drop shadow). Do **not** stack multiple gradients or card shadows; rely on subtle borders/spacing instead of layered surfaces.
